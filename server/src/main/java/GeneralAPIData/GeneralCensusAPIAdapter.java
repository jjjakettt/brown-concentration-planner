...
public class GeneralCensusAPIAdapter {
  ...
  public GeneralCensusAPIAdapter(String input_variable) {
  ...
  public Map<String, String> queryCensusAPI(String stateName, String countyName) throws Exception {
    ...
    urlBuilder.append("?get=NAME," + input_variable);
    ...
    try {
      ...
      throw new Exception("Error: Input variable is not valid", e);
    } finally {
   ...
   */
  public GeneralEntry fetchData(String key) throws Exception {
    ...
    // broadband data)
    String broadbandValueStr = dataMap.get(input_variable);
    System.out.println(dataMap);
    System.out.println("broadbandValueStr: " + broadbandValueStr);

    if (broadbandValueStr != null) {
      double broadbandValue = Double.parseDouble(broadbandValueStr); // Parse the broadband value
      return new GeneralEntry(stateName, countyName, broadbandValue);
    }

    // Create and return a new BroadbandEntry with the fetched data
    return new GeneralEntry(stateName, countyName, null);
  }
}
