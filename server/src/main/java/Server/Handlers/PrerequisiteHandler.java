...
import WebScraper.Course;
import WebScraper.Prerequisites;
...
public class PrerequisiteHandler implements Route {
    ...
    public Object handle(Request request, Response response) {
        try {
            ...
            return new PrereqResponse(prereqText != null, prereqText == null ? List.of() : List.of(prereqText)).serialize();
            
        } catch (Exception e) {
            e.printStackTrace();
            return new ErrorResponse("error_invalid_request", 
                "Could not process prerequisite check request: " + e.getMessage()).serialize();
        }
    }
    ...
    private String getPrerequisiteText(String courseCode) {
        try {
            ...
            String prereqText = fullText.substring(startIndex + "Prerequisites:".length()).trim();
    ...
    // Response records
    public record PrereqResponse(
        String response_type,
        boolean prerequisites_met,
        List<String> missing_prerequisites
    ) {
        public PrereqResponse(boolean met, List<String> missing) {
        ...
        String serialize() {
            try {
                ...
                return moshi.adapter(PrereqResponse.class).toJson(this);
            } catch (Exception e) {
