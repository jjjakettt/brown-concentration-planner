...
/**
 * Interface for the props that are passed into SelectHistory.
 *
 * @params
...
export function SelectHistory(props: SelectHistoryProps) {
   ...
   * @param inputDataset - The dataset to display in a table format.
  ...
  function createTable(inputDataset: Array<Array<string>>) {
    ...
    // Trim the inputDataset after "Total Credits"
    ...
    >
      {
        props.history.length !== 0 ? (
          <div>{createTable(props.history)}</div>
        ) : null
      }
    </div>
  );
